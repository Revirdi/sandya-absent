<x-app-layout>
    <form action="{{ route('globals.update', $global) }}" method="POST" autocomplete="off"
        class="p-6 max-w-xl rounded-lg shadow-md space-y-4 bg-gray-200">
        @csrf
        @method('PUT')
        <h2 class="text-2xl font-bold" style="margin-bottom: 20px">Edit Global Setting</h2>

        <!-- Name -->
        <div>
            <label class="block mb-1 font-medium">Name</label>
            <input type="text" name="name" value="{{ old('name', $global->name) }}"
        ...
        <div>
            ...
            <input type="text" name="value" value="{{ old('value', $global->value) }}"
                class="w-full px-3 py-2 rounded bg-gray-100 border border-gray-300 focus:outline-none focus:border-blue-500"
                required>
            @error('value')
                <p class="text-red-500 mt-1 text-sm">{{ $message }}</p>
